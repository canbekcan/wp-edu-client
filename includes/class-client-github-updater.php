...
class WP_EDU_Client_Github_Updater {
    ...
    public function __construct( $repo_user, $repo_name, $plugin_file ) {
        ...
        $this->plugin_basename = plugin_basename( $plugin_file ); // örn: wp-edu-client/wp-edu-client.php
        $this->plugin_slug     = dirname( $this->plugin_basename ); // örn: wp-edu-client
    ...
    private function get_github_release() {
        ...
        delete_transient( $cache_key ); // Test aşamasında önbelleği eziyoruz
    ...
    public function check_for_update( $transient ) {
        ...
        if ( version_compare( $current_version, $new_version, '<' ) ) {
            ...
            // WordPress'in doğru eşleştirmesi için anahtar plugin_basename olmalıdır
    ...
    public function fix_github_folder_name( $source, $remote_source, $upgrader ) {
        ...
        if ( $wp_filesystem->exists( $source_trail . $main_file ) ) {
            $expected_folder = trailingslashit( $remote_source ) . $this->plugin_slug;
